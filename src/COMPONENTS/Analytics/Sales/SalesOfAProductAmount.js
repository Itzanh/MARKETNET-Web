...
class SalesOfAProductAmount extends Component {
    ...
    draw(rows) {
        ...
        // key: product id, value: array of float64 amount
        const data = {};
        const labels = [];
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            for (let j = 0; j < this.list.length; j++) {
                const amount = rows[i].amount[this.list[j].id];
                data[this.list[j].id].push(amount != null ? amount : 0);
            }
        }
    ...
    locateProducts() {
        ...
        ReactDOM.render(<LocateProduct
            ...
            onSelect={(product) => {
                ...
                this.salesOfAProductAmount(productIds).then((data) => {
                    this.draw(data);
                });
            }}
    ...
    render() {
        return <div id="tabSalesOfAProductAmount" className="formRowRoot">
            ...
            <h4>{i18next.t('sales-of-a-product-amount')}</h4>
